...
export default async function NewReleasesPage() {
  ...
  const rows = await db
    .select({
      ...
      rating: books.rating, reviewCount: books.reviewCount, authorName: authors.name, isNewArrival: books.isNewArrival,
    ...
    <div className="bg-[#f8f9fc] min-h-screen">
      ...
      <main id="main" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h1 className="text-2xl font-bold text-[#0f172a] mb-1 flex items-center gap-2">
          <Sparkles className="w-6 h-6 text-[#2d5a9e]" /> New Releases
        </h1>
        <p className="text-[#475569] text-sm mb-8">Recently added to our catalog</p>

        {rows.length === 0 ? (
          <div className="bg-white rounded-xl border border-[#e2e8f0] p-16 text-center">
            <p className="text-[#475569]">No books available yet.</p>
          </div>
        ) : (
